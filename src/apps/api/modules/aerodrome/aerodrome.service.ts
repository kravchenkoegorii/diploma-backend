...
import { MINUTE, SECOND } from 'src/common/constants/time';
import {
  ILock,
  IPosition,
  IVotingReward,
...
export class AerodromeService {
  ...
  async getLiquidityPositions(
    ...
    try {
      ...
      for (let i = 0; i < multicallResults.length; i++) {
        const result = multicallResults[i].result;
        if (result) {
  ...
  async getExtendedLocksByAddress(
    ...
    const aero_info = tokenList?.find((token) => token.symbol === 'AERO');
    ...
    let extendedLocks: ILock[] = locks.map((lock) => ({
      ...lock,
      token_symbol: lock.token === aero_info.token_address ? 'AERO' : 'VELO',
    ...
    switch (type) {
      ...
      case LockOperations.ClaimLockRewards:
        const aeroPrice = Number(aero_info.price) || 0;
        extendedLocks = extendedLocks.filter((lock) => {
          const lockRewards = +formatUnits(
            lock.rebase_amount,
            aero_info.decimals,
          );
          const usdAmount = lockRewards * aeroPrice;
          return (
  ...
  async getPositionsSummary(
    ...
    try {
      ...
      const [
        ...
        this.getWalletRewards(chainId, walletAddress),
        this.getWalletRewards(chainId, walletAddress, block24hAgo),
      ]);
  ...
  async getExtraPoolData(chainId: number, pool: PoolData, address: string) {
    ...
    return await viemClient.multicall({
      contracts: [
        ...
        {
          address: pool?.emissions_token as Address,
          abi: erc20Abi,
          functionName: 'symbol',
        },
        {
  ...
  async getTopTokens({
    ...
    const aerodromeTokens = await this.cacheService.get<IToken[]>(
      getTokenInfoKey(chainId),
    );

    if (!aerodromeTokens?.length) {
      return [];
    }
    ...
    await Promise.all(
      aerodromeTokens.map(async (token, index) => {
        ...
        if (token.listed && (!token.market_cap || !token.volume_24h)) {
          ...
          if (tokenInfo?.marketCap)
            aerodromeTokens[index]['market_cap'] = tokenInfo.marketCap;
          if (tokenInfo?.volume24h)
            aerodromeTokens[index]['volume_24h'] = tokenInfo.volume24h;
        }
      }),
    );
  ...
  private async formatPositionChunk(
    ...
    for (let i = 0; i < positionsChunk.length; i++) {
      ...
      const pool = pools.find((pool) => pool.lp === position.lp);
      ...
      positionsChunkFormatted.push({
        ...
        poolBalance0: data[4].result,
        poolBalance1: data[5].result,
        accountBalance0: data[6].result,
        accountBalance1: data[7].result,
        emissionsTokenAddress: pool?.emissions_token as Address,
        emissionsTokenSymbol: data[8].result,
        emissionsTokenDecimals: data[9].result,
        emissionsEarned,
        emissionsEarnedUSD: floorToFixed(
          +emissionsEarned * +(emissionsToken?.price || 0),
        ),
