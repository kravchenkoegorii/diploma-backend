...
} from '../../../../common/utils/bigint';
import { parseBigIntToString } from '../../../../common/utils/parse-big-int-to-string';
...
@Injectable()
export class VelodromeStatisticsService {
  private readonly logger = new Logger(VelodromeStatisticsService.name);

  private readonly EPOCH_DURATION = 7 * 24 * 60 * 60 * 1000; // 7 days in ms
  private readonly START_EPOCH = 150;
  private readonly START_DATE = new Date(Date.UTC(2025, 3, 10, 0, 0, 0)); // 10 april 2025 00:00 UTC
  ...
  public async findPoolsWithFilters({
    ...
    if (filters?.isExotic) {
      const exoticPools = filteredPools.filter(
        (pool) =>
          !pool.symbol.includes('USDC/VELO') &&
  ...
  public async getTopRelaysData(
    ...
    try {
      ...
      allRelaysData = allRelaysData.filter(
        (relay) => !relay.inactive && relay.name.includes('veVELO'),
      );
  ...
  public async getCurrentVotingRound(chainId: number): Promise<EpochData> {
    ...
    const now = new Date();
    const elapsedEpochs = Math.floor(
      (now.getTime() - this.START_DATE.getTime()) / this.EPOCH_DURATION,
    );
    const epochCount = this.START_EPOCH + elapsedEpochs;
  ...
  async getRebaseApr(chainId: number): Promise<IRebaseAprData> {
    const viemClient = this.viemService.getViemClient(OPTIMISM_ID);

    const { votingEscrow, minter } = chainsConfig[OPTIMISM_ID];
  ...
  async addMoreDataToPools(
    ...
    const opTokens = await this.cacheService.get<TokenResponse[]>(
      getTokenInfoKey(OPTIMISM_ID),
    );
    ...
    const veloToken = opTokens.find((token) => token.symbol === 'VELO');

    if (!veloToken) {
      throw new Error('VELO token not found.');
    }
    ...
    tokensMap.set(veloToken.token_address.toLowerCase(), veloToken);
    ...
    for (const pool of pools) {
      try {
        ...
        pool.poolUrl = chainsScansHelper(chainId, pool.lp);
        ...
        if (
          (token0?.token_address.toLowerCase() ===
            '0x7F5c764cBc14f9669B88837ca1490cCa17c31607'.toLowerCase() ||
            token1?.token_address.toLowerCase() ===
              '0x7F5c764cBc14f9669B88837ca1490cCa17c31607'.toLowerCase()) &&
          chainId === OPTIMISM_ID
        ) {
          const type =
            pool.type === 0 ? 'sAMM' : pool.type === -1 ? 'vAMM' : 'CL';
          pool.symbol =
            type === 'CL'
              ? `${type}${pool.type}-${token0?.symbol}/${token1?.symbol}`
              : `${type}-${token0?.symbol}/${token1?.symbol}`;
        }
  ...
  async getStatistics(chainIds: number[]) {
    const results = await Promise.all(
      chainIds.map(async (chainId) => {
        const [pools, tokens, relays] = await Promise.all([
          this.cacheService.get<PoolData[] | undefined>(
            getPoolsDataKey(chainId),
          ),
          this.cacheService.get<TokenResponse[] | undefined>(
            getTokenInfoKey(chainId),
          ),
          this.cacheService.get<IRelay[] | undefined>(getRelaysKey(chainId)),
        ]);
    ...
    const {
      ...
      (acc, curr) => ({
        totalTvl: acc.totalTvl + curr.chainTvl,
        totalSwapFees: acc.totalSwapFees + curr.chainSwapFees,
        totalVolume: acc.totalVolume + curr.chainVolume,
        totalListedTokens: acc.totalListedTokens + curr.listedTokens,
        totalPools: acc.totalPools + curr.poolsLength,
        totalRelays: acc.totalRelays + curr.relaysLength,
      }),
      {
        totalTvl: 0,
        totalSwapFees: 0,
    ...
    return {
      ...
      //we add the length of the networks since the native token is added to the list of tokens of each network on the dashboard
      totalListedTokens: totalListedTokens + chainIds.length,
