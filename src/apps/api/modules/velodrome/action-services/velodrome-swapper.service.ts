...
@Injectable()
export class VelodromeSwapperService {
  private readonly logger = new Logger(VelodromeSwapperService.name);

  constructor(
    private readonly tokensService: TokensService,
    private readonly viemService: ViemService,
    private readonly routesService: VelodromeRoutesService,
  ...
  async swap(
    ...
    const swapIdForLogs = `[chainId: ${chainId}] ${crypto.randomUUID()}`;
    ...
    const token0Contract = !isFromETH
      ? getContract({
    ...
    let token1Price = +(tokenOut.price || 0);
    ...
    const feeBn = parseUnits(
      fee.toFixed(chain.nativeCurrency.decimals),
      ethToken.decimals,
    );

    const gasPrice = await viemClient.getGasPrice();
    const AVERAGE_GAS_PER_TX = BigInt(450000);
    ...
    if (isFromETH) {
      ...
      const nativeCurrencyBalance = await viemClient.getBalance({
        address: fromAddress,
      });
      this.logger.log(
        `[Swap: ${swapIdForLogs}]: Balance of ${fromAddress} is ${nativeCurrencyBalance} ${token0Symbol}. Required amount is ${amountInBn} ${token0Symbol}`,
      );

      const amountDiff =
        +nativeCurrencyBalance.toString() - +amountInBn.toString();
      shouldForceMax =
        Math.abs(amountDiff) < +nativeCurrencyBalance.toString() * 0.01;
      ...
      if (shouldForceMax) {
        ...
        amountInBn =
          ...
          feeBn -
          gasPrice * AVERAGE_GAS_PER_TX * forceMaxRatio;
        amountIn = +formatUnits(nativeCurrencyBalance, token0decimals);
      }

      if (nativeCurrencyBalance < amountInBn) {
        // Check if balance is enough
        const message =
          `Not enough native Currency Balance ${token0Symbol} to pay for the transaction.\n` +
      ...
      const token0Balance =
        (await token0Contract?.read.balanceOf([fromAddress])) || BigInt(0);
      this.logger.log(
      ...
      if (!token0Balance) {
        const message =
          `Not enough ${token0Symbol} to pay for the transaction.\n` +
          `Please, top up your wallet to proceed transaction.`;
        this.logger.error(`[Swap: ${swapIdForLogs}]: ${message}`);
        return {
          success: false,
    ...
    // Get actual amount out from Velodrome
    ...
    if (amountOut <= amountOutExpected * 0.7) {
      this.logger.error(
        `[Swap: ${swapIdForLogs}]: Amount out returned from Velodrome is less than expected. Swap will result in money loss`,
      );
      return {
        success: false,
        message: `Velodrome returned amount out much less than expected. Swap will result in money loss.`,
      };
    }
    ...
    if (isFromETH) {
      ...
      try {
        // Estimate gas for the transaction
        const allowance = await token0Contract?.read.allowance([
          fromAddress,
          universalRouter,
        ]);

        if (allowance && allowance < amountInBn) {
          const approvalGas = await token0Contract?.estimateGas.approve(
