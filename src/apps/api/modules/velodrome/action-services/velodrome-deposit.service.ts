...
import { additionalChaionsRouterAbi } from '../../../../../common/constants/chains/abis/additional-chaions-router.abi';
import { OPTIMISM_ID } from '../../../../../common/constants/chains/optimism';

@Injectable()
export class VelodromeDepositService {
  private readonly logger = new Logger(VelodromeDepositService.name);
  ...
  async addLiquidityToLp(
    ...
    const ethToken = await this.tokensService.getTokenBySymbol(
      this.velodromeService.getNativeTokenSymbol(chainId),
      chainId,
    );
    ...
    const isEthPool =
      tokenInInfo.symbol.toUpperCase() ===
        this.velodromeService.getNativeTokenSymbol(chainId) ||
      tokenOutInfo.symbol.toUpperCase() ===
        this.velodromeService.getNativeTokenSymbol(chainId);
    const isFromEth =
      tokenInInfo.symbol.toUpperCase() ===
      this.velodromeService.getNativeTokenSymbol(chainId);
    ...
    const { fee, feeBn } = this.velodromeService.calculateFee(
      amountIn,
  ...
  private async quoteAddLiquidity(
    ...
    try {
      ...
      const contract = getContract({
        address: router,
        abi: chainId !== OPTIMISM_ID ? additionalChaionsRouterAbi : routerAbi,
        client: viemClient,
      });
  ...
  private async computeAmounts(
    ...
    if (pool.type > 0) {
      ...
      if (tokenInUpper === poolToken0Upper) {
        ...
        amountBDesiredBN = parseUnits(
          amount.toFixed(tokenOutInfo.decimals || 18),
          tokenOutInfo.decimals || 18,
        );
      ...
      if (tokenInUpper === poolToken0Upper) {
        ...
        amountBDesiredBN = parseUnits(
          amountOut.toFixed(tokenOutInfo.decimals),
          tokenOutInfo.decimals || 18,
        );
      } else {
        const amountIn =
    ...
    const amountAToApproveBN =
      amountADesiredBN +
      BigInt(Math.trunc(+amountADesiredBN.toString() * 0.15));
    const amountBToApproveBN =
      amountBDesiredBN +
      BigInt(Math.trunc(+amountBDesiredBN.toString() * 0.15));
