...
@Injectable()
export class VelodromeClaimerService {
  private readonly logger = new Logger(VelodromeClaimerService.name);
  ...
  > {
    ...
    try {
      ...
      const userPositions = await this.velodromeService.getLiquidityPositions(
        chainId,
        walletAddress,
        null,
        false,
      ...
      for (let i = 0; i < positions.length; i++) {
        try {
          ...
          const { feeBn: first } = this.velodromeService.calculateFee(
          ...
          const { feeBn: second } = this.velodromeService.calculateFee(
  ...
  > {
    ...
    try {
      ...
      const cachePositions = await this.velodromeService.getLiquidityPositions(
        chainId,
        walletAddress,
        null,
        false,
