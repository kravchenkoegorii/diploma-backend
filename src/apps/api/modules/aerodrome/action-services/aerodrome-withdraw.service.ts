...
@Injectable()
export class AerodromeWithdrawService {
  private readonly logger = new Logger(AerodromeWithdrawService.name);
  ...
  async withdrawAMMPoolLiquidity(
    ...
    try {
      ...
      try {
        const lpTokenContract = await getContract({
          abi: erc20Abi,
