...
@Injectable()
export class VelodromeVoterService {
  private readonly logger = new Logger(VelodromeVoterService.name);
  ...
  //TODO: need to test on each network, as there is different from the base chain behavior
  async claimVotingRewards(
    ...
    const votingRewards = await this.velodromeService.getVotingRewards(
      chainId,
      walletAddress as Address,
    );
