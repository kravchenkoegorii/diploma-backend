...
@Injectable()
export class VelodromeRoutesService {
  public static readonly VOLATILE_SPACE = 4194304;
  public static readonly STABLE_SPACE = 2097152;
  private readonly logger = new Logger(VelodromeRoutesService.name);
  ...
  async findShortestSwapRoute(
    ...
    const mixedQuoterContractAddress = chainsConfig[chainId].mixedQuoter;
    ...
    const poolsForSwaps = pools.filter((p) => +(p.tvl ?? 0) > 10);
    ...
    const mixedQuoterContract = getContract({
      address: mixedQuoterContractAddress,
      abi: mixedQuoterAbi,
      client: viemClient,
    });
    ...
    for (let i = 0; i < nodes.length && i < 25; i++) {
      ...
      for (let j = 0; j < node.length; j++) {
        ...
        if (pool.type === -1) {
          path.push(VelodromeRoutesService.VOLATILE_SPACE);
        } else if (pool.type === 0) {
          path.push(VelodromeRoutesService.STABLE_SPACE);
        } else {
      ...
      try {
        ...
        this.logger.error('Error quoting path', e);
      }
    }
  ...
  private getPathTypesAndValues(
    ...
    return {
      ...
      values: nodes.reduce((accumulated, node) => {
        let feeTier =
          Number(node.type) === 0
            ? VelodromeRoutesService.STABLE_SPACE
            : VelodromeRoutesService.VOLATILE_SPACE; // Choose fee based on type.
