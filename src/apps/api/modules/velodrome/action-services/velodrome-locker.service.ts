...
import { IWithdrawLock } from 'src/common/interfaces/actions/withdraw-lock.interface';
import { IClaimLock } from 'src/common/interfaces/actions/claim-lock.interface';
...
@Injectable()
export class VelodromeLockerService {
  private readonly logger = new Logger(VelodromeLockerService.name);
  ...
  async increaseLockTokens(
    ...
    try {
      ...
      const userLocks = await this.velodromeService.getLocksByAddress(
        chainId,
        walletAddress,
      );
      ...
      const { feeBn, fee } = this.velodromeService.calculateFee(
        amount,
  ...
  async lockTokens(
    ...
    try {
      ...
      const { feeBn } = this.velodromeService.calculateFee(
        amount,
        Number(tokenInfo.price || '0'),
        ethPrice,
        yamlConfig.FEE_DETAILS.FEE_PCT,
      ...
      const currentEpochData =
        await this.velodromeStatsService.getCurrentVotingRound(chainId);
  ...
  async resetLock(
    ...
    try {
      ...
      const res = await this.velodromeService.getExtendedLocksByAddress(
        chainId,
        walletAddress,
  ...
  async pokeLock(
    ...
    try {
      ...
      const locks = await this.velodromeService.getLocksByAddress(
        chainId,
        walletAddress,
      );
  ...
  async setLockToRelay(
    ...
    try {
      ...
      const relays = await this.velodromeStatsService.getTopRelaysData(
        chainId,
        walletAddress as Address,
      );
  ...
  async withdrawLock(
    ...
    try {
      if (
        ...
        !isNumber(chainId) ||
        !isString(lockId) ||
        !['VELO'].includes(token.toUpperCase())
      ) {
        this.logger.error(
