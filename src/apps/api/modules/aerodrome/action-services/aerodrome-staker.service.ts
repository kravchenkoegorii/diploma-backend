...
@Injectable()
export class AerodromeStakerService {
  private readonly logger = new Logger(AerodromeStakerService.name);
  ...
  async stakeLp(
    ...
    try {
      ...
      const positions = await this.aerodromeService.getLiquidityPositions(
        chainId,
        walletAddress,
        null,
        false,
      ...
      const { feeBn: firsTokenFeeBn } = this.aerodromeService.calculateFee(
      ...
      const { feeBn: secondTokenFeeBn } = this.aerodromeService.calculateFee(
      ...
      const ethToken = tokens?.find((t) => t.symbol.toUpperCase() === 'WETH');
