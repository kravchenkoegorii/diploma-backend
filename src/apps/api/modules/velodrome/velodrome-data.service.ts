...
import { BaseError, formatUnits, getContract, zeroAddress } from 'viem';
...
import { HOUR, SECOND } from '../../../../common/constants/time';
...
@Injectable()
export class VelodromeDataService {
  private readonly logger = new Logger(VelodromeDataService.name);
  ...
  async getDexTokensInfo(chainId: number): Promise<void> {
    this.logger.log(`Chain ${chainId}: start updating Velodrome tokens info`);
    ...
    try {
      ...
      //received value decimals depend on the decimals of each tokens on the pair
      const decimals = 18 + (18 - stableToken.decimals);
      const USDCToETHRateValue = +formatUnits(USDCToETHRate, decimals);
      ...
      if (wrappedETH) {
        ...
        eth.symbol = this.velodromeService.getNativeTokenSymbol(chainId);
      ...
      this.logger.log(
        `Chain ${chainId}: end updating Velodrome tokens info in ${timeDiff}s`,
      );
    }
  }

  async getDexData(chainId: number): Promise<void> {
    this.logger.log(`Chain ${chainId}: start updating Velodrome data`);
    const timeStart = Date.now();
    ...
    try {
      ...
      while (hasMoreData) {
        ...
        allPoolsData.push(
          ...poolsChunk.map((pool) => {
            ...
            const updatedSymbol = pool.symbol.includes('vAMMV2')
              ? pool.symbol.replace('vAMMV2', 'vAMM')
              : pool.symbol.includes('sAMMV2')
              ? pool.symbol.replace('sAMMV2', 'sAMM')
              : pool.symbol;

            return {
              ...pool,
              tokenPrice0: token0?.price,
              tokenPrice1: token1?.price,
              chainId,
      ...
      const poolsWithAdditionalData =
        await this.velodromeStatisticsService.addMoreDataToPools(
          chainId,
          parsedData,
          rewards || [],
        );
      ...
      const timeDiff = (timeEnd - timeStart) / 1000;
      this.logger.log(
        `Chain ${chainId}: end updating Velodrome data in ${timeDiff}s`,
      );
    }
  }

  async getDexEpochsLatest(chainId: number): Promise<void> {
    this.logger.log(`Chain ${chainId}: start updating Velodrome rewards data`);
    ...
    try {
      ...
      const timeDiff = (timeEnd - timeStart) / 1000;
      this.logger.log(
        `Chain ${chainId}: end updating Velodrome rewards data in ${timeDiff}s`,
      );
    }
  }
}
